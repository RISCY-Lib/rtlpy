...
class AddressBlock (_named_design) :
  def __init__(self, name:str, regWidth:int) :
    self.name = name
  ...
  def regWidth(self) -> int :
    """The width of the registers in the address block in bits
    """
    return self._regWidth

  @regWidth.setter
  def regWidth(self, val:int) -> None :
    if (not isinstance(val, int)) :
      raise ValueError(f"{type(self)}.regWidth must be int")

  def get_reg_width_bytes(self) -> int :
    """Returns the register width in bytes (instead of native bits)

    Returns:  
        int: The register width in bytes
    """
    return math.ceil(self.regWidth / 8)
  
  def add_reg(self, reg:Register) -> None :
    """Adds a register to the AddressBlock. Checks the register's provided
    offset does not overlap with an existing registers. Also checks that 
    the register is not larger than the regWidth of the address block

    Args:
        reg (Register): The register to add to the AddressBlock
    """
    # TODO: Add support for dimensioned registers
    if (reg.msb_pos() > self.regWidth) :
      raise ValueError(f"Can't add oversized register ({reg.name}, width={reg.msb_pos()}) to address-block ({self.name}, regWidth={self.regWidth})")

    rwb = self.get_reg_width_bytes()

    for r in self.regs :

      if (_overlaps((reg.offset, reg.offset + rwb), (r.offset, r.offset + rwb))) :
        raise ValueError(f"Can't add reg named {reg.name} to address-block named {self.name}. Overlaps with existing reg (named {r.name}).")
      if (reg.name == r.name) :
        raise ValueError(f"Cant add reg named {reg.name} to address-block named {self.name}. A register of that name already exists.")

    self.regs.append(reg)

  def append_reg(self, reg:Register) :
    raise NotImplementedError("AddressBlock.append_reg not implemented")
